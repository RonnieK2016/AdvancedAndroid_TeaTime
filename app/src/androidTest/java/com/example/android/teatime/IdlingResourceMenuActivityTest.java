...
public class IdlingResourceMenuActivityTest {
    ...
    // COMPLETED Registers any resource that needs to be synchronized with Espresso before
    // the test is run.
    @Before
    ...
    // COMPLETED Test that the gridView with Tea objects appears and we can click a gridView item
    @Test
    public void idlingResourceTest() {
        onData(anything()).inAdapterView(withId(R.id.tea_grid_view)).atPosition(0).perform(click());
    }

    // COMPLETED Unregister resources when not needed to avoid malfunction
    @After
